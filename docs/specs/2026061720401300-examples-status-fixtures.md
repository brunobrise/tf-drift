...
updated_at: 2026-06-17T20:40:13Z
status: approved
---
...
- `drift-new-resource`: valid Terraform using the built-in `terraform_data` resource, expected to report `DRIFTED` before state exists.
...
- The report includes one `CLEAN`, one `DRIFTED`, and one `ERROR` result.
